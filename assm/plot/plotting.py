...
import numpy as np
from assm.utility.utils import hdi, bci
...
def plot_posterior(x,
    ...
    **kwargs : dict
    ...
    if show_intervals is not None:
        ...
        print('{} [{}, {}]'.format(np.mean(x), low, high))
...
def plot_grouped_mean_prediction(x,
    ...
    if hue is None:
        ...
        if show_data:
            ...
            # data_se = [np.std(data.loc[(data[x] == j).values, y_data]) for j in x_order]
            # ax.errorbar(x_order, data_mean,
            #             yerr=data_se, 
            #             fmt='o', color='black', ms=20)
            ax.scatter(x_order, data_mean, s=150, color='black')
        ...
        if show_intervals is not None:
            ...
            mean_prediction = [np.mean(predictions.loc[(j, slice(None)), y_predictions]) for j in x_order]

            trans = Affine2D().translate(shift, 0.0) + ax.transData
            err_l = mean_prediction-low
            err_u = high-mean_prediction
            err_l[err_l<0] = 0 
            err_u[err_u<0] = 0 
            ax.errorbar(x_order, mean_prediction,
                        yerr=[err_l, err_u],
                        transform=trans, fmt=symbol, mfc='none', 
                        color=color, ms=20)
...
def plot_grouped_quantiles_prediction(predictions,
    ...
    if show_data:
        for j, l in enumerate(hue_order):
            q_data = [np.nanpercentile(data.loc[data[grouping_var] == l, 'rt'], q=x) for x in percentiles]
            # q_data_low = [np.nanpercentile(data.loc[np.logical_and(data[grouping_var] == l,
            #                                                        data.Right == 0), 'rt'], q=x) for x in
            #               percentiles]
    ...
    if model == 'ddm':
        ...
        axes[0].xaxis.label.set_fontsize(30)
        axes[0].yaxis.label.set_fontsize(30)
        # axes[1].set_ylabel('RTs Left option')

    axes[0].legend(frameon=True, loc='upper left') # bbox_to_anchor=(0, 1)
