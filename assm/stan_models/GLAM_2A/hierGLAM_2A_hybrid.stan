...
data {
	...
	array [N] int<lower=1,upper=2> choice;				// 1-> correct, 2->incorrect
...
transformed parameters {
  vector<lower=0>[N] drift_left_t;				// trial-by-trial drift rate for predictions
  vector<lower=0>[N] drift_right_t;				// trial-by-trial drift rate for predictions
  ...
  transf_mu_ndt = log(1 + exp(mu_ndt));
    ...
    A1 = G1[n] * (val1[n] + lambda_sbj[sbj[n]]) + gam_sbj[sbj[n]] * (1 - G1[n]) * val1[n];
    A2 = G2[n] * (val2[n] + lambda_sbj[sbj[n]]) + gam_sbj[sbj[n]] * (1 - G2[n]) * val2[n];

    drift_left_t[n]  = v_sbj[sbj[n]] / (1 + exp(-tau_sbj[sbj[n]] * (A1 - A2)));
    drift_right_t[n] = v_sbj[sbj[n]] / (1 + exp(-tau_sbj[sbj[n]] * (A2 - A1)));
  }
}
